...
if True:
    ...
    cv.imshow('Harris Corner Detector',img0)
    ...
    cv.imshow('combine two images with SIFT',img_combine)
    if cv.waitKey(0) & 0xff == ord("q"):
        cv.destroyAllWindows()
    ...
    cv.imshow("Detected Images Colour with rectangle", img2)
    if cv.waitKey(0) & 0xff == ord("q"):
        cv.destroyAllWindows()
...
def object_track(cap):
    ...
    while True:
        ...
        if success:
            ...
            # 1, First filter out the red object
            # Colour Detection
            bgr_img = img
            hsv_img = cv.cvtColor(bgr_img, cv.COLOR_BGR2HSV)
            ...
            def red_hsv():
                ...
                # # S -> 175 - 255
                # # V -> 20 - 255
                # lower_hsv_2 = np.array([160, 175, 20])
                # higher_hsv_2 = np.array([180, 255, 255])
            ...
            # 2. track the filtered out object
            gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
            # binarize the image
            ret, bw = cv.threshold(gray, 128, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU)
            # ret, bw = cv.threshold(gray, 0, 128, cv.THRESH_BINARY_INV + cv.THRESH_OTSU)
            bw = cv.bitwise_not(bw) # invert the matrix to get the object insight
            ...
            # Only draw rectangle for the biggest object
            color = (0,0,255)
            if nb_components > 1:
                k=1
                for i in range(1, nb_components):
                    # use if sizes[i] >= min_size: to identify objects
                    # color = np.random.randint(255,size=3)
                    if stats[i][2]  * stats[i][3] > stats[1][2]  * stats[1][3]:
                        k = i
                cv.rectangle(img2, (stats[k][0],stats[k][1]),(stats[k][0]+stats[k][2],stats[k][1]+stats[k][3]), (0,255,0), 2)
                img2[output == k + 1] = color
                ...
                # objectPositionX = (stats[0][0] + stats[0][2])/2
                # objectPositionY = (stats[0][1] + stats[0][3])/2
...
object_track(cv.VideoCapture("Square_Rotation_resize.mp4"))
object_track(cv.VideoCapture("RedBox_Resize.mp4"))
object_track(cv.VideoCapture(0))
